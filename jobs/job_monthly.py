...
def main() -> None:
    ...
    lock = JobLock(database, "job_monthly")
    if not lock.acquire():
        raise SystemExit("job_monthly is already running")
    logger = JobRunLogger(database)
    notifier = build_job_alert_notifier(config)
    run_id = logger.start("job_monthly", "monthly", "monthly", config.jobs.max_workers, config.jobs.timeout_seconds)
    stage_logger = JobStageLogger(database, run_id, "job_monthly")
    table_logger = JobTableLogger(database, run_id, "job_monthly")
    ...
    def on_stage(stage_name, duration_seconds, status, extra, message):
        try:
            ...
            print(f"job_monthly stage log warning: {stage_exc}")
    ...
    try:
        ...
        summary = service.run_monthly_sync(stage_callback=on_stage, checkpoint_store=checkpoint)
        print_sync_summary(summary, job_name='job_monthly')
        table_logger.log_summary(summary)
        finish_meta = logger.finish("success", "monthly Eastmoney concept members + share_float sync complete")
