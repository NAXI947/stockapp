...
def main() -> None:
    ...
    lock = JobLock(database, "job_quarterly")
    if not lock.acquire():
        raise SystemExit("job_quarterly is already running")
    logger = JobRunLogger(database)
    notifier = build_job_alert_notifier(config)
    run_id = logger.start("job_quarterly", "quarterly", "quarterly", config.jobs.max_workers, config.jobs.timeout_seconds)
    stage_logger = JobStageLogger(database, run_id, "job_quarterly")
    table_logger = JobTableLogger(database, run_id, "job_quarterly")
    ...
    def on_stage(stage_name, duration_seconds, status, extra, message):
        try:
            ...
            print(f"job_quarterly stage log warning: {stage_exc}")
    ...
    try:
        ...
        summary = service.run_quarterly_sync(stage_callback=on_stage)
        print_sync_summary(summary, job_name='job_quarterly')
        table_logger.log_summary(summary)
        finish_meta = logger.finish("success", "quarterly fin_indicator sync complete")
