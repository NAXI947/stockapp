...
def main() -> None:
    ...
    lock = JobLock(database, "job_yearly")
    if not lock.acquire():
        raise SystemExit("job_yearly is already running")
    logger = JobRunLogger(database)
    notifier = build_job_alert_notifier(config)
    run_id = logger.start("job_yearly", "yearly", "yearly", config.jobs.max_workers, config.jobs.timeout_seconds)
    stage_logger = JobStageLogger(database, run_id, "job_yearly")
    table_logger = JobTableLogger(database, run_id, "job_yearly")
    ...
    def on_stage(stage_name, duration_seconds, status, extra, message):
        try:
            ...
            print(f"job_yearly stage log warning: {stage_exc}")
    ...
    try:
        ...
        summary = service.run_yearly_sync(stage_callback=on_stage)
        print_sync_summary(summary, job_name='job_yearly')
        table_logger.log_summary(summary)
        finish_meta = logger.finish("success", "yearly strategy rebuild complete")
